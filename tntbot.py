...
# TODO add list of channels being watched to user messages
# TODO should report, stop and on finished backup the attendance?
...
class AttendanceCog(commands.Cog):
    ...
    def backup_reports(self, interrupted: bool = False):
        ...
        for name, (job, task) in self.jobs.items():
            ...
            title = f"Backup of task `{name}` that was watching between `{fmt_datetime(job.start)}` and `{fmt_datetime(job.end)}`\n"
            try:
    ...
    async def job_finished(self, job: WatchJob):
        ...
        await ctx.send(f"`{job.name}` final attendance")
