...
# TODO this bot assumes display_names are unique, but they don't have to be, but 7cav wouldn't allow 2 people with the same name. change?
...
class AttendanceCog(commands.Cog):
    ...
    async def poll(self, ctx: commands.Context, channel_name: str):
        ...
        if channel_name in self.poll_cache:
            ...
            lines.append(f"{len(current)} members in '{channel_name}':\n")
            for name in current:
                lines.append(f"{name}")
    ...
    @commands.command()
    async def watch(self, ctx: commands.Context, name: str, start: str, duration: str, *channel_names: str):
        now = datetime.now(TZ)
