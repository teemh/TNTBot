...
# TODO add channel list to attendance_start command
# TODO make google sheets output
# TODO time format h:m:s
# TODO at least 40 minutes
...
class AttendanceCog(commands.Cog):
    ...
    async def watch_status(self, ctx):
        if (self.taking_attendance):
            out = (f"Watching from {self.watch_start} to {self.watch_end} on the follow channels: ")
            for name in self.watch_channels.keys():
                out = out + f"{name} "
            await ctx.send(out)
        elif (self.waiting):
            await ctx.send(f"Waiting for {self.watch_start} before starting.")
        else:
    ...
    async def watch_start(self, ctx, start, end, *channels):
        await __watch_start(ctx, start, end, *channels)
    ...
    async def __watch_start(self, ctx, start, end, *channels):
        ...
        watch_start = datetime.strptime(start, "%H:%M")
        watch_end = datetime.strptime(end, "%H:%M")
        if not watch_start:
        ...
        self.watch_start = datetime.now(TZ).replace(hour=watch_start.hour, minute=watch_start.minute, tzinfo=TZ)
        self.watch_end = datetime.now(TZ).replace(hour=watch_end.hour, minute=watch_end.minute, tzinfo=TZ)

        if self.watch_start <= now:
            await ctx.send(f"ERROR: {start} can't be in the past.")
            return

        if self.watch_end <= self.watch_start:
            return
    ...
    async def before_check_attendance(self):
        ...
        if now < self.watch_start:
            self.waiting = True
            wait_seconds = (self.watch_start - now).total_seconds()
            print(f"Waiting {wait_seconds:.0f}s until start time ({self.watch_start})")
    ...
    async def tnt_message(self, ctx):
        start = int(self.watch_start.timestamp()) 
        end = int(self.watch_end.timestamp()) 
        message = f"""
