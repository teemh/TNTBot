...
class AttendanceCog(commands.Cog):
    ...
    async def list_backups(self, ctx: commands.Context):
        try:
            files = [f[:-4] for f in os.listdir('./logs') if f.endswith('.log')]
            if not files:
            ...
            LOG.error(f"ERROR: Failed to list logs: {e}")

    @commands.command()
    async def read_backup(self, ctx: commands.Context, name: str):
        # Name validation
        ...
        try:
            ...
            title = f"Backup `{name}.log`, last modified `{fmt_datetime(modified)}`\n"
            with open(filename, "r") as f:
                content = f.read()
            if not content:
                return await ctx.send(f"Log for `{name}` is empty.")
            await ctx.send(title + content)
        except FileNotFoundError:
            await ctx.send(f"ERROR: No log file found for `{name}`.")
        except OSError as e:
